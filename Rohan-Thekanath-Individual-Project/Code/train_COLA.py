...
df_train = pd.read_csv("in_domain_train.tsv", delimiter='\t', header=None, names=['sentence_source', 'label', 'label_notes', 'sentence'])

df_validation = pd.read_csv("in_domain_dev.tsv", delimiter='\t', header=None, names=['sentence_source', 'label', 'label_notes', 'sentence'])
...
# And Padding to length 67
train = tokenizer(list(train_sentences), padding=True)
train_input_ids = train['input_ids']
train_masks = train['attention_mask']

validation = tokenizer(list(validation_sentences), padding=True)
...
torch.save(model.state_dict(), "model_COLA.pt")
...
######################## Predicting on test data to get MCC value #######################
